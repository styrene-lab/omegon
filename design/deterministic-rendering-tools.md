...
aliases = ["deterministic-rendering-tools"]
...
[data]
issue_type = "feature"
open_questions = ["Should these be core tools or an extension? Core means always available; extension means optional install but process-isolated.", "Should we auto-detect installed CLI tools (d2, mmdc, pandoc) at session start and only expose tools that have backends?", "Should rendered output go to a file, return as data URI in tool result, or both?"]
parent = "null"
priority = "3"
+++
