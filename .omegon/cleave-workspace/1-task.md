---
task_id: 1
label: startup-and-status-audit
siblings: [0:auth-core-audit, 2:consumer-surface-audit]
---

# Task 1: startup-and-status-audit

## Root Directive

> Perform a full sweep of the auth and login subsystems: audit storage, provider mapping, startup hydration, status surfaces, logout, refresh flows, and consumer checks for consistency; fix any concrete issues found, add tests, and commit the changes.

## Mission

Audit and fix startup hydration and status/reporting surfaces so persisted credentials, environment export, and auth status outputs are consistent across runs. Cover setup, main/control auth commands, and add tests where practical.
