...
- [x] 2.1 Implement `DiscoveryScheduler` with per-endpoint TTL state (Copilot TTL from `refresh_in`/`expires_at`; default 1h configurable), refresh triggers: startup post-auth, explicit refresh (TTL bypass), TTL expiry
...
- [x] 3.3 Surface per-provider freshness (fresh/cached/stale + last-confirmed timestamp) and an explicit TTL-bypassing refresh action in the model selection surface (`tui/mod.rs` model list + `control_runtime.rs` consumers)
...
- [x] 4.1 Live-verify available credentialed endpoints (github-copilot and anthropic); fixture-verify openai, groq, mistral, xai, openrouter, and google and record live verification as deferred until accounts are available
- [x] 4.2 Keep openai-codex `discovery: none`: no synchronously resolvable ChatGPT-backend token is available and unproven enumeration must not be enabled
- [x] 4.3 Inspect live Copilot `/models` body shape; confirm capabilities/limits mapping in `CopilotDiscovery` matches reality
- [x] 4.4 Update CHANGELOG with 0.28.2 section; full `just test-rust` + `just lint` green; release binary linked
