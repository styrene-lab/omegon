...
- [ ] 3.1 Rework the login task (main.rs ~5000): `begin_login` before spawn, `complete_login(Succeeded)` or `complete_login(Failed{timeout|stale_state_only|refused})` at terminal outcome — map from accept_oauth_callback errors and token-exchange failures
- [ ] 3.2 Failed login reverts route to `prior` and sets a persistent footer warning (cleared on next attempt), not only a SystemNotification
