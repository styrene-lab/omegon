...
[data]
title = "Omegon Console Backend Surface"
status = "exploring"
issue_type = "epic"
priority = 2
dependencies = []
...
Build the backend substrate for a future pure-Omegon Dioxus console: a web/desktop UI that talks to a long-running `omegon serve` daemon. The near-term work is backend-first: stabilize headless API and ACP session surfaces before building the Dioxus UI.

The console should not be a Flynt clone and should not depend on Flynt document/runtime assumptions. It should be an Omegon-native surface over Omegon daemon services.
...
```text
Dioxus Omegon Console
  ├─ ACP-over-WebSocket for interactive agent sessions
  └─ Omegon-native HTTP/WebSocket API for runtime, lifecycle, settings, memory, and project state
...
ACP is the agent conversation/session protocol. The native API is the application control plane.
...
ACP ext_methods remain useful for Zed/Flynt/editor compatibility. The Dioxus app should use a normal Omegon API for app state and embed ACP only where agent-session semantics are needed.
...
apps/omegon-console/           # future Dioxus app, after backend surfaces settle
```

If workspace constraints favor crates only, the first app can live under:

```text
core/crates/omegon-console/
```
...
The browser/Dioxus app acts as an ACP client for conversation sessions. This should reuse existing ACP semantics rather than inventing a separate chat stream.
...
### Substrate evaluation: persistent agent console pattern
...
Architectural direction: use ACP-over-WebSocket strictly for interactive agent sessions and conversation semantics; use native HTTP/WebSocket daemon APIs for runtime, lifecycle, memory, settings, tasking, run history, and dashboard state. Durable autonomy should be modeled as persistent executable objectives: task becomes actionable via trigger, sentry claims it, agent executes with bounded spec/checkpoints/budget, results update lifecycle/memory/evidence, and the console shows truth.

Implications for this design node: first backend substrate work should stabilize read DTOs and control-plane APIs before Dioxus UI. The near-term foundation remains ACP lifecycle read expansion plus native HTTP mirrors. The strategic follow-on is durable task/sentry APIs, not messaging-gateway breadth. Messaging, remote/container backends, and multimodal adapters are later ingress/egress plugins over the core substrate, not the core product.
...
This changes the assistant assessment: the superior assistant should not bake every capability into the core console. Core Omegon should provide the durable runtime, policy, lifecycle, memory, tasking, ACP sessions, and extension loading/control plane. Skills/plugins/extensions provide replaceable capabilities and domain expertise. The console should expose this ecosystem as an operator surface: discover installed/available assets, show capability health, configure secrets/settings, explain trust/permissions, enable/disable skills or extensions, and bind them to durable agent tasks or assistant profiles.
...
- **Image generation/creative tools** — `scry` demonstrates heavyweight local generation as an extension with models, galleries, previews, secrets, and config. The console needs generic extension widget/config/status surfaces so this class does not require bespoke UI.
...
Strategic implication: building our own superior assistant means composing a durable harness plus an inspectable capability graph. The console should answer: what can this assistant do, why is it allowed, what knowledge does it carry, what tasks is it running, what extensions/skills are active, what secrets/config are missing, and what evidence supports its claims. That is a stronger substrate than broad hard-coded adapters.
...
This changes the console backend shape again: the upstream discovery API should not expose only `extensions | plugins | skills | agents`. The assistant app needs a normalized **capability catalog** with at least these classes:
...
6. Surface Armory distribution/index artifacts later so the console can browse cached/signed catalog snapshots instead of live GitHub traversal only.

## Decisions

### Decision: Backend-first, Dioxus later

**Status:** decided
**Rationale:** The Dioxus UI should consume stable daemon APIs. Building the UI first would force it to depend on unstable file formats or internal tool handlers.

### Decision: Use ACP plus native API

**Status:** decided
**Rationale:** ACP is the right session protocol; native API is the right application control plane for a browser/Dioxus app.
...
The document correctly keeps ACP for conversation/session semantics and native APIs for app control-plane state. Missing detail: ACP session creation needs a way to select an assistant profile and receive the profile/capability envelope that shaped the session. Otherwise the console cannot explain why a session had particular tools, skills, memory, or restrictions.
...
The local `.omegon/evidence` substrate and deterministic TDD savepoint extension suggest a stronger trust model for autonomous work. The current node does not require the console to show evidence records, claim provenance, or verification artifacts. That would waste a differentiator against adapter-heavy systems.
...
- [assumption] The first Dioxus app should live under `apps/omegon-console/` instead of `core/crates/omegon-console/`.
