...
- **Governed schema v12 migration**: Startup migrates schemas v5-v11 before opening the project store. Historical v5/v6 `default` records move to `legacy`; post-v7 `default` records move to `primensus`. Schema v12 permits confirmed inferences to become active with retained attribution. Legacy records do not gain fabricated confirmation; legacy vectors remain unverified until repair.
