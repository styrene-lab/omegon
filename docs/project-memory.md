...
- **Governed schema v10 migration**: Startup migrates schemas v5-v9 before opening the project store. Historical v5/v6 `default` records move to `legacy`; post-v7 `default` records move to `primensus`. Schema v10 retains formation metadata and receipts while adding nullable vector identity/source fields. Legacy vectors remain unverified until repair.
