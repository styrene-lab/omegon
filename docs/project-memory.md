...
- **Governed schema v11 migration**: Startup migrates schemas v5-v10 before opening the project store. Historical v5/v6 `default` records move to `legacy`; post-v7 `default` records move to `primensus`. Schema v11 retains formation and vector metadata while adding nullable lifecycle-inference attribution. Legacy facts retain unknown attribution; legacy vectors remain unverified until repair.
