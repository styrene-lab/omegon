...
impl InventorySnapshot {
        ...
        for (id, patch) in layer.deployments {
            if let Some(existing) = self.deployments.get_mut(&id) {
                if let Some(value) = patch.provider {
                    existing.provider = Evidenced::new(value, source, evidence);
                }
                if let Some(value) = patch.interface {
                    existing.interface = Evidenced::new(value, source, evidence);
                }
                if let Some(value) = patch.base_url {
                    existing.base_url = value.map(|v| Evidenced::new(v, source, evidence));
                }
                ...
                let Some(provider) = patch.provider else {
                    return Err(vec![format!("deployment '{}' lacks provider", id.0)]);
                };
                let Some(interface) = patch.interface else {
                    return Err(vec![format!("deployment '{}' lacks interface", id.0)]);
                };
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        for deployment in self.deployments.values() {
            if !self.providers.contains_key(&deployment.provider.value) {
                errors.push(format!(
                    "deployment '{}' references unknown provider '{}'",
                    deployment.id.0, deployment.provider.value.0
                ));
            }
        }
        for offering in self.offerings.values() {
            if !self.deployments.contains_key(&offering.deployment.value) {
                errors.push(format!(
                    "offering '{}' references unknown deployment '{}'",
                    offering.id.0, offering.deployment.value.0
                ));
            }
    ...
    fn rejection_reasons(
        ...
        if !deployment.enabled.value || !offering.enabled.value {
            reasons.push(RejectionReason::Disabled);
        }
        if deployment.interface.value != request.interface {
...
fn offering_from_patch(
    ...
    let Some(deployment) = patch.deployment else {
        return Err(vec![format!("offering '{}' lacks deployment", id.0)]);
    };
...
fn apply_offering_patch(
    ...
    if let Some(value) = patch.deployment {
        offering.deployment = Evidenced::new(value, source, evidence);
    }
...
mod tests {
    ...
    fn layer_with_offering(source: InventorySource, output: &str, graded: bool) -> InventoryLayer {
        ...
        layer.providers.insert(
            ProviderIntegrationId("lab".into()),
            ProviderPatch {
                display_name: Some("Lab".into()),
            },
        );
        layer.deployments.insert(
            EndpointDeploymentId("lab-chat".into()),
            DeploymentPatch {
                provider: Some(ProviderIntegrationId("lab".into())),
                interface: Some(InferenceInterface(
                    InferenceInterface::CHAT_COMPLETIONS.into(),
                )),
                enabled: Some(true),
                ..Default::default()
            },
