...
mod tests {
    ...
    #[test]
    fn demo_steps_count_is_correct() {
        // Verify step order: Verify should come before Power Tools in demo mode
        let verify_idx = STEPS_DEMO.iter().position(|s| s.title == "Verify and Launch");
        let power_idx = STEPS_DEMO.iter().position(|s| s.title == "Power Tools");
        assert!(verify_idx.is_some(), "Verify and Launch step missing from STEPS_DEMO");
        assert!(power_idx.is_some(), "Power Tools step missing from STEPS_DEMO");
        assert!(
            verify_idx.unwrap() < power_idx.unwrap(),
            "Verify and Launch ({}) must come before Power Tools ({}) in STEPS_DEMO",
            verify_idx.unwrap(), power_idx.unwrap()
        );
    }
    ...
    fn demo_command_step_allows_check_command_to_advance() {
        // STEPS_DEMO has STEP_WEB_DASHBOARD with Trigger::Command("dash")
        let mut tut = Tutorial::new_demo(true);
        while !matches!(tut.step().trigger, Trigger::Command(_)) {
            tut.advance();
        }
        let idx = tut.step_index();
        assert_eq!(tut.step().title, "Web Dashboard");
