...
- Catalog `omegon-nex` read-only delegation metadata for `devenv.inspect`, `devenv.explain`, and `machine-profile.inspect` and thread that catalog into `nex_substrate` reports, while keeping direct Nex binary inspection as an explicit advisory fallback.
