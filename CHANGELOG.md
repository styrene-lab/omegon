...
- Unified `/update install` with the graceful runtime restart lifecycle: successful replacement now queues the same shutdown, session finalization, provider shutdown, terminal teardown, and process re-exec path as `/runtime restart` instead of executing from a background download task.
