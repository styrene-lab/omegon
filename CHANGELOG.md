...
- Add a provider-neutral core evidence read model that loads `.omegon/evidence` claims, records, and edges, summarizes claim support/refutation, annotates OpenSpec scenarios with `evidence-claim` support metadata, and reports refuted/mixed scenario evidence claims for future repo-rule enforcement without hard-blocking core archive operations.
