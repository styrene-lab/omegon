...
- Applied a selected profile's model intent to the live route controller, so choosing an existing named profile changes runtime routing instead of only updating persisted/shared settings; selected profile files without an embedded display name now retain their registry ID in status output.
