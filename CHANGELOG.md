...
- Document the next native-headroom live A/B task matrix covering log, diff, retrieval, markdown, code, and debug-from-log task shapes.
...
- Tighten automated headroom dogfood fact extraction so broad corpus eval can enforce a zero-restoration gate without treating incidental list/search tokens as required facts.
...
- Cap `codebase_search` result counts at 50 with requested/effective metadata as the first bounded-output step for ranked navigation tools.
...
- Fix headroom A/B report token-delta wording so increased on-run token usage is reported as an increase rather than mislabeled as savings.
