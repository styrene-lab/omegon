...
- Added `GET /api/events` as a read-only daemon event queue snapshot and `GET /api/events/stream` as a Server-Sent Events daemon/app update stream alongside the existing authenticated `POST /api/events` ingress.
