...
- Add a provider-neutral core evidence read model that loads `.omegon/evidence` claims, records, and edges, summarizes claim support/refutation, annotates OpenSpec scenarios with `evidence-claim` support metadata, and blocks archive attempts for explicitly refuted scenario evidence claims.
