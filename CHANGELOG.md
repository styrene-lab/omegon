...
- Add and harden a dogfood Rust surface evidence generator that uses nightly rustdoc JSON to populate `.omegon/evidence/surfaces.jsonl`, resolve source paths/hashes, filter derive noise, render basic signatures, append `surface-map` evidence records, emit source/artifact edges, report Rust doc coverage warnings, and build a derived SQLite/FTS query index, and connect documentation coverage evidence to an explicit claim record.
